...
import scrapy
from banks.scraper.items import BranchItem
from common.scraper.items import BaseLoader
from common.scraper.utils import format_url, normalize_space
from scrapy.loader.processors import Compose, Identity, Join, MapCompose


class Loader(BaseLoader):
    default_item_class = BranchItem
...
class Spider(scrapy.Spider):
    ...
    def parse_items(self, response, region_name, region_name_full):
        for branch_elem in response.css('.list__main .list__item:not(.list__item--header)'):
            ...
            loader.add_css('id_self_banki', '::attr(data-id)')
            loader.add_css('url_bank_banki', '.item__name__bank::attr(href)')
